...
États : LANE_FOLLOWING → PARKING_DETECTED → APPROACHING → ALIGNING → REVERSE_PARKING → WAITING → EXIT → LANE_FOLLOWING
"""
...
class StateMachine:
    ...
    def transition(self, new_state: State):
        rospy.loginfo(
            f"[StateMachine] {self.state.value} → {new_state.value}"
        )
        self.state = new_state

    # ------------------------------------------------------------------
    # Méthodes de test d'état
    # ------------------------------------------------------------------
    ...
    def is_approaching(self):
        return self.state == State.APPROACHING
    ...
    def on_start_approach(self):
        """Avancer un peu après détection pour se positionner."""
        if self.is_parking_detected():
            self.transition(State.APPROACHING)

    def on_approach_done(self):
        """Positionnement terminé, début de l'alignement."""
